...
describe('Test CheTheiaPluginsAnalyzer', () => {
  ...
  let metaYamlGenerator: MetaYamlGenerator;
  ...
  beforeEach(() => {
    ...
    container.bind(MetaYamlGenerator).toSelf().inSingletonScope();
    container.bind(Sidecar).toConstantValue(sidecar);
    sideCarGetDockerImageFor.mockResolvedValue(fakeImage);
    metaYamlGenerator = container.get(MetaYamlGenerator);
  });
  afterEach(() => {
  ...
  test('basics', async () => {
    ...
    const result = await metaYamlGenerator.compute(cheTheiaPluginMetaInfos);
  ...
  test('no package.json', async () => {
    ...
    await expect(metaYamlGenerator.compute(cheTheiaPluginMetaInfos)).rejects.toThrow('No package.json found for');
  });
  ...
  test('no publisher field in package.json', async () => {
    ...
    await expect(metaYamlGenerator.compute(cheTheiaPluginMetaInfos)).rejects.toThrow(
  ...
  test('no icon field in package.json', async () => {
    ...
    const cheTheiaPluginMetaInfos: CheTheiaPluginMetaInfo[] = [cheTheiaPluginMetaInfo];
    await metaYamlGenerator.compute(cheTheiaPluginMetaInfos);
  ...
  test('no creationDate', async () => {
    ...
    await expect(metaYamlGenerator.compute(cheTheiaPluginMetaInfos)).rejects.toThrow('No creation date found for vsix');
  });
});
